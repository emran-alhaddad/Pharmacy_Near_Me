...
class PharController extends Controller
{
  ...
  public function activity($id, $state)
  {
    ...
    if ($affected && $state == 0) {
      $email_data = [
        'name' => $user->name,
        'message' => 'لقد تم تعطيل حسابك على منصة علاجي من قبل إدارة الموقع'
      ];
      Mail::to($user->email)->send(new UserActivationEmail($email_data));
      return back()->with('status', 'تم تعطيل حساب الصيدلية');
    } else {
      $email_data = [
        'name' => $user->name,
        'message' => 'لقد تم تفعيل حسابك على منصة علاجي من قبل إدارة الموقع'
      ];
      Mail::to($user->email)->send(new UserActivationEmail($email_data));
